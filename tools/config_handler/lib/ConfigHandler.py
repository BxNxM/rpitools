...
config_path = os.environ["RPITOOLS_CONFIG"]
template_config_path = os.environ["RPITOOLS_CONFIG_TEMPLATE"]
    ...
    def __replace_magic_variables(self, parameter):
        if "$" in parameter:
            for env_var, env_val in os.environ.items():
                env_var_in_conf = "${}".format(env_var).strip()
                if len(env_var_in_conf) <= 2:
                    continue
                if env_var_in_conf in parameter:
                    parameter = parameter.replace(env_var_in_conf, env_val)
        return parameter
